...
public class TST
{
     ...
     * @return: Associated with the key if found, -1 if not found.
     */
    protected int get(TSTNodes node, String key)
    {
        char c = key.charAt(0);
        if (node == null)
        {   
     ...
     * @return: void.
     */
    private void traverse(TSTNodes node, String string)
    {
        if (node != null)
        {
     ...
     * @return: A TSTNodes.
     */
    protected TSTNodes put(TSTNodes node, String key, int value)
    {
        char c = key.charAt(0);
        if (key.length() > 1)
        {
